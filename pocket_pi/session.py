...
class SessionManager:
    ...
    def list_sessions(cls, cwd: str) -> List[Tuple[str, str, float]]:
        ...
        """
        cwd_dir_name = "--" + str(Path(cwd).resolve()).replace("/", "-").replace("\\", "-").strip("-") + "--"
        project_dir = Path("~/.pocket_pi/agent/sessions").expanduser() / cwd_dir_name
                    ...
                    with open(f, "r", encoding="utf-8") as file:
                        for line in file:
                            ...
                            if entry.get("type") == "session_info":
                                ...
                                break
                            elif entry.get("type") == "message" and not display_name or display_name == "Untitled Session":
                                ...
                                if message.get("role") == "user":
                                    ...
                                    display_name = (content[:50] + "...") if len(content) > 50 else content
                except Exception:
                    pass
    ...
    def new_session(self, name: Optional[str] = None):
        ...
        if not self.session_file:
            self.session_file = self.project_session_dir / f"{timestamp_str}_{session_uuid}.jsonl"
    ...
    def append_message(self, role: str, content: Any, thinking: Optional[str] = None, usage: Optional[Dict] = None, tool_call_id: Optional[str] = None, tool_name: Optional[str] = None) -> str:
        """
        ...
        if compaction_entry:
            ...
            for entry in path:
                if entry["id"] == first_kept_id:
                    keep = True
                if keep and entry["id"] != compaction_entry["id"]: # Skip the compaction marker itself
