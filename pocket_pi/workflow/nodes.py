    ...
    def post(self, shared, prep_res, user_input):
        ...
        if user_input.startswith("!"):
            ...
            if not command.strip():
                console.print(
                    "[bold red]Please specify a bash command, e.g. !ls[/bold red]"
                )
                return "input_again"
            ...
            output = run_tool(
                "bash", {"command": command}, cwd=str(shared["session"].cwd)
            )
            ...
            shared["session"].append_message(role="bashExecution", content=bash_msg_obj)
            return "input_again"
        ...
        if user_input.startswith("/"):
            ...
            if cmd in ["/quit", "/exit"]:
                ...
                if not skill_content:
                    console.print(
                        f"[bold red]Skill '{skill_name}' not found.[/bold red]"
                    )
                    return "input_again"
                ...
                console.print(
                    f"[bold green]Loaded skill instructions: {skill_name}[/bold green]"
                )
                ...
                if arguments:
                    ...
                    console.print(
                        f"[bold green]Skill '{skill_name}' successfully loaded into conversation context. Please ask your question now![/bold green]"
                    )
                    return "input_again"
                ...
                console.print(
                    f"[bold red]Unknown command:[/bold red] {cmd}. Type /help for available actions."
                )
                return "input_again"
...
class ClearNode(Node):
    ...
    def post(self, shared, prep_res, exec_res):
        console.print(
            "[green]Conversation chat history cleared to a clean slate![/green]"
        )
    ...
    def post(self, shared, prep_res, help_content):
        console.print(
            Panel(
                ...
                title="[bold cyan]Pocket-Pi Manual[/bold cyan]",
                border_style="cyan",
            )
        )
        return "loop"
...
class LoginNode(Node):
    ...
    def post(self, shared, prep_res, result):
        if result:
            ...
            console.print(
                f"[green]Successfully saved API key for [bold]{result['provider']}[/bold] in settings.json[/green]"
            )
        return "loop"
...
class ModelNode(Node):
    ...
    def exec(self, current):
        ...
        console.print(
            f"Current Thinking Level: [cyan]{current['current_thinking']}[/cyan] (off/minimal/low/medium/high/xhigh)"
        )
    ...
    def post(self, shared, prep_res, result):
        ...
        if "thinking_level" in result:
            ...
            console.print(
                f"[green]Thinking level changed to: {result['thinking_level']}[/green]"
            )
            print_session_info(shared)
            ...
            console.print(
                f"[green]Swapped model permanently for this project to: {result['provider']}/{result['model']}[/green]"
            )
            print_session_info(shared)
    ...
    def post(self, shared, prep_res, selected_file):
        if selected_file:
            shared["session"] = SessionManager(
                cwd=str(prep_res), session_file_path=selected_file
            )
            console.print(
                f"[green]Successfully loaded session log: {shared['session'].get_session_name()}[/green]"
            )
            print_session_info(shared)
        return "loop"
...
"""
    ...
    def post(self, shared, prep_res, details):
        console.print(
            Panel(
                Markdown(details), title="[bold green]Active Session Stats[/bold green]"
            )
        )
        return "loop"
...
class NewSessionNode(Node):
    ...
    def exec(self, cwd):
        try:
            name = input(
                "Enter display name for new session (or press Enter): "
            ).strip()
    ...
    def post(self, shared, prep_res, name):
        ...
        if name:
            shared["session"].append_session_info(
                name
            )  # Store as type: "session_info" correctly!
...
class CompactNode(Node):
    ...
    def prep(self, shared):
        ...
        return {"context": context, "path": path, "config": shared["config"]}
    ...
    def exec(self, data):
        ...
        compaction_messages = [
            {
                "role": "user",
                "content": f"Please summarize this conversation history briefly. Focus only on the goals accomplished and code edits made:\n\n{serialize_text}",
            }
        ]
    ...
    def post(self, shared, prep_res, result):
        if result == "skipped":
            ...
            console.print(
                f"[green]Successfully compacted session. Summary saved.[/green]"
            )
    ...
    def prep(self, shared):
        ...
        if user_msg:
            if isinstance(user_msg, list):
                user_msg = "".join(
                    [m.get("text", "") for m in user_msg if m.get("type") == "text"]
                )
            ...
            has_path_marker = (
                "/" in user_msg_lower or "." in user_msg_lower or "\\" in user_msg_lower
            )
...
"""
        ...
        # 4. Inject local AGENT.md instructions if present
        agent_md_path = Path(cwd_str) / "AGENT.md"
        if agent_md_path.exists():
            try:
                agent_instructions = agent_md_path.read_text(encoding="utf-8").strip()
                if agent_instructions:
                    system_prompt += f"\n\nAdditional User Instructions (from AGENT.md):\n{agent_instructions}"
                    log_debug(
                        f"[PlannerNode] Loaded additional instructions from {agent_md_path}"
                    )
            except Exception as e:
                ...
                while t.is_alive():
                    if fd:
                        ...
                        if r:
                            ...
                            if (
                                char == "\x1b" or char == "\x03"
                            ):  # \x1b is Escape, \x03 is Ctrl+C
    ...
    def post(self, shared, prep_res, result):
        if "error" in result:
            ...
            if session.current_leaf_id:
                parent_id = session.entries.get(session.current_leaf_id, {}).get(
                    "parentId"
                )
                if parent_id:
            ...
            log_debug(
                f"[PlannerNode] Connection failure: {result['error']}. Routing back to prompt loop."
            )
            return "loop"
        ...
        if result.get("tool_calls"):
            log_debug(
                f"[PlannerNode] Routing to Executor with {len(result['tool_calls'])} tool call(s)..."
            )
            return "tools"
...
def prompt_gatekeeper_choice(category: str, item: str) -> str:
    ...
    if not sys.stdin.isatty():
        console.print(
            "[yellow]Non-interactive terminal detected. Defaulting to: Block once.[/yellow]"
        )
        return "block_once"
...
def check_and_prompt_permissions(cwd: str, name: str, args: dict) -> Tuple[bool, str]:
    ...
    for cmd in commands_to_check:
        ...
        if status == "block":
            return (
                False,
                f"Permission Denied: Command '{cmd}' is blocked in this project.",
            )
        elif status == "allow":
            continue
        else:
            ...
            if decision == "allow_once":
                ...
                return (
                    False,
                    f"Permission Denied: Execution of command '{cmd}' was blocked by the user.",
                )
    ...
    for url in urls_to_check:
        ...
        if status == "block":
            return (
                False,
                f"Permission Denied: Access to url/host '{url}' is blocked in this project.",
            )
        elif status == "allow":
            continue
        else:
            ...
            if decision == "allow_once":
                ...
                return (
                    False,
                    f"Permission Denied: Access to url/host '{url}' was blocked by the user.",
                )
            elif decision == "always_block":
...
class ExecutorNode(Node):
    ...
    def prep(self, shared):
        log_debug(
            "[ExecutorNode] Preparing tool execution environment with Gatekeeper..."
        )
        ...
        return {"tool_calls": gatekeeper_tool_calls, "cwd": cwd}
    ...
    def exec(self, data):
        ...
        for tc in data["tool_calls"]:
            ...
            if blocked_reason:
                ...
                console.print(
                    f"\n[cyan]🛠️ Calling Tool:[/cyan] [bold]{name}[/bold]("
            ...
            if blocked_reason:
                console.print(
                    f"  [bold red]✘[/bold red] [bold red]{name}[/bold red] blocked by Gatekeeper."
                )
            else:
                console.print(
                    f"  [bold green]✔[/bold green] [bold cyan]{name}[/bold cyan] completed. Size: [cyan]{size_kb} KB[/cyan] | Lines: [cyan]{line_count}[/cyan]"
                )
    ...
    def post(self, shared, prep_res, exec_results):
        ...
        for res in exec_results:
            ...
            tool_content = [{"type": "text", "text": res["output"]}]
            shared["session"].append_message(
                role="toolResult",
        ...
        log_debug(
            "[ExecutorNode] Tool execution completed. Routing back to Planner Node..."
        )
        return "default"
